...
static int read_symbol_table32(symbol_table_info *table_info) {
    ...
    for (size_t i = 0; i < table_info->symbols_number; i++) {
        ...
        if (ELF32_ST_TYPE(symbol_header->st_info) != STT_FILE &&
            ELF32_ST_TYPE(symbol_header->st_info) != STT_SECTION &&
            ...
            symbol->nm_type = symbol_nm_type(ELF32_ST_BIND(symbol_header->st_info),
...
int handle32(Elf32_Ehdr *elf_header, symbol_table_info *symbol_table_info) {
    ...
    return handle_error_prefix(ERRMSG_NOSYMTAB, "handle32", EXIT_FAILURE);
}