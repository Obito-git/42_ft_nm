...
static void table_info_init(symbol_table_info *table_info, enum Sort sort, enum Display display) {
    table_info->symbols = NULL;
    table_info->noname_symb = NULL;
    table_info->added_symbol_count = 0;
    table_info->added_noname_count = 0;
    table_info->display = display;
    table_info->sort = sort;
}
...
int handleElf(void *file_start, const char *file_name, bool print_name, enum Sort sort, enum Display display) {
    ...
    if (ptr[0] == ELFMAG0 && ptr[1] == ELFMAG1
        ...
        table_info_init(&table_info, sort, display);
